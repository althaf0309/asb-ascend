...
import {
  ...
  topicKeywordsForLocation, topicPath,
} from '@/data/locations';
...
const LocationCourse = () => {
  ...
  useEffect(() => {
    ...
    setPageSeo({ title: `${title} | ASB Training Hub`, description, keywords: keywords.join(', '), path, image: hero1536 });
  ...
  return <main>
    <section className="relative min-h-[620px] pt-28 flex items-center overflow-hidden bg-foreground">
      <picture className="absolute inset-0"><source media="(max-width: 640px)" srcSet={hero640} /><source media="(max-width: 1200px)" srcSet={hero1200} /><img src={hero1536} alt={`${topic.shortName} students from ${location.name}`} className="h-full w-full object-cover" fetchPriority="high" /></picture>
